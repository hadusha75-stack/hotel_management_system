...
if (isset($_POST["loadrooms"])) {
    ...
    $stmt = $conn->prepare("SELECT roomnumber, price FROM rooms WHERE bedtype=? AND roomtype=? AND status='not booked'");
    $stmt->bind_param("ss", $selectedBed, $selectedRoom);
    $stmt->execute();
    $res = $stmt->get_result();
    while ($row = $res->fetch_assoc()) {
        $availableRooms[]  = $row["roomnumber"];
        $availableprices[] = $row["price"];
    }
    $stmt->close();
    $roomsLoaded = true;
}
...
if (isset($_POST["checkin_submit"])) {
    ...
    $priceperday  = trim($_POST["price"]        ?? "");
    $availableRooms  = $_POST["room_options"]  ?? [];
    $availableprices = $_POST["price_options"] ?? [];
    $selectedBed  = $bedtype;
    ...
    else { foreach (preg_split('/\s+/', $name) as $p) {
    ...
    if (empty($mobilenumber) || !preg_match('/^(\+?251[79]\d{8}|0[79]\d{8})$/', $mobilenumber))
    ...
    if (empty($checkin) || strtotime($checkin) < strtotime(date("Y-m-d")))
        $field_errors["checkin"] = "Check-in date must be today or in the future.";
    if (empty($roomnumber))  $field_errors["roomnumber"] = "Please select a room.";
    if (empty($priceperday)) $field_errors["price"]      = "Please select a price.";

    if (empty($field_errors)) {
        $stmt = $conn->prepare("INSERT INTO customer (name,email,mobilenumber,nationality,gender,address,idproof,bedtype,roomtype,roomnumber,checkin,priceperday) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("ssssssssssss", $name,$email,$mobilenumber,$nationality,$gender,$address,$idproof,$bedtype,$roomtype,$roomnumber,$checkin,$priceperday);
        if ($stmt->execute()) {
            $upd = $conn->prepare("UPDATE rooms SET status='booked' WHERE roomnumber=?");
            $upd->bind_param("s", $roomnumber);
            $upd->execute(); $upd->close();
            $checkin_done = true;
            $success_msg = "You have successfully checked in to Room <strong>$roomnumber</strong>.";
        } else {
            $error_msg = "Check-in failed: " . $stmt->error;
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .step-label-top{font-size:10.5px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.3);margin-bottom:14px;}
        ...
        .strip-fields select:focus{outline:none;border-color:var(--gold);background:rgba(255,255,255,0.12);}
        .btn-load{display:inline-flex;align-items:center;gap:8px;padding:11px 24px;background:linear-gradient(135deg,var(--blue-dark),var(--blue));color:#fff;border:none;border-radius:var(--radius-sm);font-size:13.5px;font-weight:600;font-family:Inter,sans-serif;cursor:pointer;transition:var(--transition);box-shadow:0 4px 14px rgba(41,128,185,0.35);white-space:nowrap;align-self:flex-end;}
        ...
        .btn-reset{display:inline-flex;align-items:center;gap:8px;padding:12px 22px;background:#f0f0f0;color:var(--text-mid);border:1.5px solid #ddd;border-radius:var(--radius-sm);font-size:14px;font-weight:600;font-family:Inter,sans-serif;cursor:pointer;transition:var(--transition);}
        ...
        .btn-back{display:inline-flex;align-items:center;gap:8px;padding:12px 28px;background:linear-gradient(135deg,var(--navy),var(--navy-mid));color:#fff;border:none;border-radius:var(--radius-sm);font-size:14px;font-weight:600;font-family:Inter,sans-serif;text-decoration:none;transition:all 0.3s ease;}
        @media(max-width:700px){.topbar{padding:0 16px;}.page-content{padding:24px 12px 40px;}.step-strip{padding:18px 16px;}.form-body{padding:20px 16px;}.grid-3,.assign-box{grid-template-columns:1fr;}.col-span-3{grid-column:span 1;}.form-footer{flex-direction:column;align-items:stretch;}.btn-checkin{justify-content:center;}}
    </style>
</head>
<body>
...
<header class="topbar">
    ...
    <a href="../public/rooms.php" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Rooms</a>
</header>

<main class="page-content">
    <div class="page-header">
        <h2><i class="fas fa-sign-in-alt" style="color:#29b6f6;margin-right:10px;font-size:24px;"></i>Guest Check In</h2>
        <p>Step 1: choose preferences &amp; load rooms &nbsp;&middot;&nbsp; Step 2: fill your details &amp; confirm</p>
...
if ($checkin_done): ?>
    <div class="card">
        <div class="success-box">
            ...
            <p><?= $success_msg ?></p>
            <a href="../public/rooms.php" class="btn-back"><i class="fas fa-home"></i> Back to Rooms</a>
        </div>
    </div>
    <?php
require_once __DIR__ . '/../core/config.php';
    ...
    <div class="card" style="margin-bottom:20px;">
        <form action="checkin.php" method="POST">
            <div class="step-strip">
                <div class="step-label-top">Step 1 — Select Room Preferences &amp; Load Available Rooms</div>
                <div class="strip-fields">
                    <div class="field">
                        <label><i class="fas fa-bed"></i> Bed Type</label>
                        <select name="bedtype">
                            <option value="Single" <?= $selectedBed==="Single"?"selected":"" ?>>Single</option>
                            <option value="Double" <?= $selectedBed==="Double"?"selected":"" ?>>Double</option>
                            <option value="Triple" <?= $selectedBed==="Triple"?"selected":"" ?>>Triple</option>
                        </select>
                    </div>
                    <div class="field">
                        <label><i class="fas fa-snowflake"></i> Room Type</label>
                        <select name="roomtype">
                            <option value="AC"    <?= $selectedRoom==="AC"   ?"selected":"" ?>>AC</option>
                            <option value="NonAC" <?= $selectedRoom==="NonAC"?"selected":"" ?>>Non-AC</option>
                        </select>
                    </div>
...
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
foreach ($availableRooms as $r): ?><span class="room-pill"><i class="fas fa-door-open"></i> <?= htmlspecialchars($r) ?></span><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endforeach; ?>
                </div>
...
require_once __DIR__ . '/../core/db.php';
                ...
                <div class="rooms-bar empty"><span class="rb-label"><i class="fas fa-times-circle" style="margin-right:5px;"></i>No rooms available for this selection. Try a different type.</span></div>
                <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endif; ?>
...
if ($roomsLoaded && !empty($availableRooms)): ?>
    <div class="card">
        <form id="guestForm" action="checkin.php" method="POST">
            <input type="hidden" name="bedtype"  value="<?= htmlspecialchars($selectedBed) ?>">
            <input type="hidden" name="roomtype" value="<?= htmlspecialchars($selectedRoom) ?>">
            <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
foreach ($availableRooms  as $r): ?><input type="hidden" name="room_options[]"  value="<?= htmlspecialchars($r) ?>"><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endforeach; ?>
            <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
foreach ($availableprices as $p): ?><input type="hidden" name="price_options[]" value="<?= htmlspecialchars($p) ?>"><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endforeach; ?>
            ...
            <div class="form-body">
                ...
                <div class="grid-3">
                    <div class="form-group">
                        ...
                        <input type="text" id="f_name" name="name" placeholder="e.g. Ashenafi Hadush" maxlength="30"
                               value="<?= htmlspecialchars($name ?? $prefillName) ?>"
                               class="<?= isset($field_errors["name"])?"is-invalid":"" ?>" oninput="liveVal(this)" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
...
require_once __DIR__ . '/../core/db.php';
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="f_email" name="email" placeholder="you@email.com" maxlength="60"
                               value="<?= htmlspecialchars($email ?? $prefillEmail) ?>"
                               class="<?= isset($field_errors["email"])?"is-invalid":"" ?>" oninput="liveVal(this)" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
...
require_once __DIR__ . '/../core/db.php';
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="f_mobile" name="mobilenumber" placeholder="09XXXXXXXX or +2519XXXXXXXX" maxlength="14"
                               value="<?= htmlspecialchars($mobilenumber ?? "") ?>"
                               class="<?= isset($field_errors["mobilenumber"])?"is-invalid":"" ?>" oninput="liveVal(this)" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
...
require_once __DIR__ . '/../core/db.php';
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="f_nat" name="nationality" placeholder="e.g. Ethiopian" maxlength="30"
                               value="<?= htmlspecialchars($nationality ?? "") ?>"
                               class="<?= isset($field_errors["nationality"])?"is-invalid":"" ?>" oninput="liveVal(this)" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
if (isset($field_errors["nationality"])): ?><span class="field-error"><i class="fas fa-exclamation-circle"></i><?= $field_errors["nationality"] ?></span><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endif; ?>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="f_id" name="idproof" placeholder="Passport / National ID" maxlength="20"
                               value="<?= htmlspecialchars($idproof ?? "") ?>"
                               class="<?= isset($field_errors["idproof"])?"is-invalid":"" ?>" oninput="liveVal(this)" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
...
require_once __DIR__ . '/../core/db.php';
                    ...
                    <div class="form-group col-span-3">
                        ...
                        <input type="text" id="f_addr" name="address" placeholder="City / Street / Full address" maxlength="100"
                               value="<?= htmlspecialchars($address ?? "") ?>"
                               class="<?= isset($field_errors["address"])?"is-invalid":"" ?>" oninput="liveVal(this)" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
if (isset($field_errors["address"])): ?><span class="field-error"><i class="fas fa-exclamation-circle"></i><?= $field_errors["address"] ?></span><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endif; ?>
                    </div>
                </div>
                <div class="section-label"><i class="fas fa-bed"></i> Room &amp; Date</div>
...
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
foreach ($availableRooms as $r): ?><option value="<?= htmlspecialchars($r) ?>"><?= htmlspecialchars($r) ?></option><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        <label><i class="fas fa-tag"></i>Price / Night (ETB)</label>
                        <select name="price" required>
                            <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
foreach ($availableprices as $p): ?><option value="<?= htmlspecialchars($p) ?>"><?= htmlspecialchars($p) ?></option><?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        <label><i class="fas fa-bed"></i>Bed Type</label>
                        <input type="text" value="<?= htmlspecialchars($selectedBed) ?>" readonly style="background:#f0f0f0;border-style:dashed;color:#4a4a6a;cursor:not-allowed;">
                    </div>
                    <div class="form-group">
                        <label><i class="fas fa-snowflake"></i>Room Type</label>
                        <input type="text" value="<?= htmlspecialchars($selectedRoom) ?>" readonly style="background:#f0f0f0;border-style:dashed;color:#4a4a6a;cursor:not-allowed;">
                    </div>
                    <div class="form-group" style="grid-column:span 2;">
                        <label><i class="fas fa-calendar-alt"></i>Check-In Date</label>
                        <input type="date" id="f_checkin" name="checkin" min="<?= date("Y-m-d") ?>"
                               value="<?= htmlspecialchars($checkin ?? "") ?>"
                               class="<?= isset($field_errors["checkin"])?"is-invalid":"" ?>" required>
                        <?php
require_once __DIR__ . '/../core/config.php';
require_once __DIR__ . '/../core/db.php';
...
require_once __DIR__ . '/../core/db.php';
            ...
            <div class="form-footer">
                <span class="hint"><i class="fas fa-info-circle" style="margin-right:4px;"></i>All fields required. Room loaded from Step 1.</span>
...
function mark(el, ok, msg) {
    ...
    let old = el.parentElement.querySelector(".js-err");
    if (old) old.remove();
    if (!ok && msg) {
        const s = document.createElement("span");
        s.className = "field-error js-err";
